...
@section('title', 'Monitoring & Notifikasi Kenaikan Pangkat Pegawai - Bakesbangpol')
...
<div class="container-fluid px-3 py-2">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('employees.promotions.print', ['month' => $selectedMonth, 'year' => $selectedYear, 'preset' => $preset]) }}" target="_blank" class="btn btn-sm btn-outline-danger d-inline-flex align-items-center">
                <i class="bi bi-printer me-1"></i>Cetak Daftar Nominatif
            </a>
            <a href="{{ route('employees.index') }}" class="btn btn-sm btn-outline-secondary d-inline-flex align-items-center">
    ...
    @if($currentMonthCount > 0)
        <div class="alert alert-warning border-warning-subtle shadow-sm rounded-4 d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4 p-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h6 class="fw-bold mb-1 text-dark">
                        Pemberitahuan: Terdapat {{ $currentMonthCount }} Pegawai Siap / Jatuh Tempo Naik Pangkat Bulan Ini!
                    </h6>
            ...
            <div>
                <a href="{{ route('employees.promotions', ['preset' => 'current_month']) }}" class="btn btn-sm btn-dark fw-semibold px-3">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3 p-md-4">
            <form action="{{ route('employees.promotions') }}" method="GET" class="row g-3 align-items-end">
                <div class="col-md-3">
                    ...
                    <select name="month" class="form-select form-select-sm">
                        @foreach($monthNames as $num => $name)
                            <option value="{{ $num }}" {{ ($selectedMonth == $num && empty($preset)) ? 'selected' : '' }}>
                                {{ $name }} (Bulan {{ $num }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    ...
                    <select name="year" class="form-select form-select-sm">
                        @for($y = 2024; $y <= 2030; $y++)
                            <option value="{{ $y }}" {{ ($selectedYear == $y && empty($preset)) ? 'selected' : '' }}>
                                Tahun {{ $y }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-md-2 d-flex gap-1">
                    ...
                    <a href="{{ route('employees.promotions') }}" class="btn btn-sm btn-outline-secondary" title="Reset Filter">
            ...
            <div class="d-flex gap-2 flex-wrap align-items-center mt-3 pt-3 border-top">
                ...
                <a href="{{ route('employees.promotions', ['preset' => 'current_month']) }}" class="badge {{ $preset === 'current_month' ? 'bg-primary text-white' : 'bg-light text-dark border' }} text-decoration-none px-3 py-2">
                    Bulan Ini ({{ \Carbon\Carbon::now()->translatedFormat('F Y') }})
                </a>
                <a href="{{ route('employees.promotions', ['preset' => 'next_month']) }}" class="badge {{ $preset === 'next_month' ? 'bg-primary text-white' : 'bg-light text-dark border' }} text-decoration-none px-3 py-2">
                    Bulan Depan ({{ \Carbon\Carbon::now()->addMonth()->translatedFormat('F Y') }})
                </a>
                <a href="{{ route('employees.promotions', ['preset' => 'next_3_months']) }}" class="badge {{ $preset === 'next_3_months' ? 'bg-primary text-white' : 'bg-light text-dark border' }} text-decoration-none px-3 py-2">
                    3 Bulan Kedepan
                </a>
                <a href="{{ route('employees.promotions', ['preset' => 'this_year']) }}" class="badge {{ $preset === 'this_year' ? 'bg-primary text-white' : 'bg-light text-dark border' }} text-decoration-none px-3 py-2">
                    Sepanjang Tahun {{ \Carbon\Carbon::now()->year }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3">
            ...
            <div class="d-flex gap-2 overflow-auto pb-2" style="white-space: nowrap;">
                @foreach($timelineProjection as $tl)
                    @php
                        $isActive = ($selectedMonth == $tl['month'] && $selectedYear == $tl['year'] && empty($preset));
                    @endphp
                    <a href="{{ route('employees.promotions', ['month' => $tl['month'], 'year' => $tl['year']]) }}" class="btn btn-sm {{ $isActive ? 'btn-primary' : ($tl['count'] > 0 ? 'btn-outline-primary' : 'btn-light text-muted border') }} text-start py-2 px-3 rounded-3" style="min-width: 100px;">
                        <div class="small fw-semibold">{{ $tl['label'] }}</div>
                        <div class="fs-6 fw-bold mt-1">
                            {{ $tl['count'] }} <span class="small fw-normal" style="font-size: 10px;">Pegawai</span>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3 col-6">
            <div class="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-4 border-primary">
                ...
                <div class="fs-4 fw-bold text-dark mt-1">{{ count($selectedPeriodEmployees) }} Pegawai</div>
                <small class="text-muted">
                    @if($preset === 'this_year') Tahun {{ $selectedYear }}
                    @elseif($preset === 'next_3_months') 3 Bulan Kedepan
                    @else {{ $monthNames[$selectedMonth] ?? 'Bulan Ini' }} {{ $selectedYear }}
                    @endif
                </small>
            </div>
        </div>
        <div class="col-md-3 col-6">
            <div class="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-4 border-danger">
                ...
                <div class="fs-4 fw-bold text-danger mt-1">{{ $countGolIV }} Orang</div>
        ...
        <div class="col-md-3 col-6">
            <div class="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-4 border-success">
                ...
                <div class="fs-4 fw-bold text-success mt-1">{{ $countGolIII }} Orang</div>
        ...
        <div class="col-md-3 col-6">
            <div class="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-4 border-info">
                ...
                <div class="fs-4 fw-bold text-info mt-1">{{ $countGolII + $countPppk }} Orang</div>
        ...
        <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
            <h6 class="fw-bold text-dark mb-0">
                ...
                <span class="badge bg-primary-subtle text-primary ms-1">
                    @if($preset === 'this_year') Tahun {{ $selectedYear }}
                    @elseif($preset === 'next_3_months') 3 Bulan Kedepan
                    @else {{ $monthNames[$selectedMonth] ?? '' }} {{ $selectedYear }}
                    @endif
                </span>
            </h6>
            <span class="small text-muted">Ditemukan <strong>{{ count($selectedPeriodEmployees) }}</strong> Pegawai</span>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="font-size: 13px;">
                    ...
                    <tbody>
                        @forelse($selectedPeriodEmployees as $idx => $emp)
                            @php
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="fw-bold text-primary">
                                        <i class="bi bi-arrow-up-circle-fill text-success me-1"></i>{{ $emp->pangkat_berikutnya_estimasi }}
                                    </span>
                                </td>
                                <td>
                                    <small class="text-muted">{{ $emp->pangkat_terakhir_tmt ? $emp->pangkat_terakhir_tmt->format('d/m/Y') : '-' }}</small>
                                </td>
                                <td>
                                    <strong>{{ $nextDate ? $nextDate->format('d/m/Y') : '-' }}</strong>
                                    <div class="small text-muted">{{ $nextDate ? $nextDate->diffForHumans() : '' }}</div>
                                </td>
                                ...
                                <td class="text-center pe-3">
                                    <div class="d-flex gap-1 justify-content-center">
                                        <a href="{{ route('employees.show', ['employee' => $emp, 'tab' => 'rank']) }}" class="btn btn-sm btn-outline-primary" title="Buka Detail & Riwayat Pangkat">
